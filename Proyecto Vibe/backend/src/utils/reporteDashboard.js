...
export async function generarReporteDashboardBuffer() {
  ...
  const resumen = [
    ...
    { Indicador: 'Fecha de exportación', Valor: fechaExportacion },
  ];
  ...
  XLSX.utils.book_append_sheet(libro, XLSX.utils.json_to_sheet(filasUsuarios), 'Usuarios');

  return {
    buffer: XLSX.write(libro, { type: 'buffer', bookType: 'xlsx' }),
    fechaExportacion,
  };
}
...
export function nombreArchivoReporte() {
  ...
  return `reporte-dashboard-${fecha}.xlsx`;
}
